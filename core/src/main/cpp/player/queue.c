...
static void packet_double_size(PacketQueue *queue) {
    ...
    if (queue->write_index == 0) {
        ...
        memcpy(temp_packets + (queue->write_index + queue->size), queue->packets + queue->write_index,sizeof(AVPacket *) * (queue->size - queue->read_index));
...
//            LOGI("packet_queue_put full cb_")
//        }
//        LOGI("packet_queue_put pthread_cond_wait - ")
//        pthread_cond_wait(queue->cond, queue->mutex);
//        LOGI("packet_queue_put pthread_cond_release - ")
    }
    ...
    queue->write_index = (queue->write_index + 1) % queue->size;
    LOGI("packet_queue_put success,write index % d stream index % d size % d",queue->write_index,packet->stream_index,queue->size)
    queue->count++;
