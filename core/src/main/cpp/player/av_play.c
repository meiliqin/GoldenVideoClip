...
static inline void set_buffer_time(AVPlayContext *context) {
    ...
    float buffer_time_length =
            context->buffer_time_length > duration ? duration : context->buffer_time_length;
...
//    memset(context, 0, sizeof(AVPlayContext));
    ...
    pthread_mutex_init(&context->context_t, NULL);
    LOGE("enter %s pthread_cond_signal", __func__);
    pthread_cond_init(&context->eof_cond_t, NULL);
    pthread_cond_init(&context->seek_t, NULL);
    av_register_all();
    avfilter_register_all();
    context->audio_filter_context = audio_filter_create();
    context->main_looper = ALooper_forThread();
    pipe(context->pipe_fd);
    if (1 != ALooper_addFd(context->main_looper, context->pipe_fd[0], ALOOPER_POLL_CALLBACK,
                           ALOOPER_EVENT_INPUT,
...
void *audio_decode_thread(void *data) {
    ...
    while (!context->abort_request) {
        ...
        if (ret == 0) {
            ...
            if (add_ret >= 0) {
                while (1) {
                    filter_ret = av_buffersink_get_frame(audio_filter_context->buffer_sink_context,
                                                         frame);
                    if (filter_ret < 0 || filter_ret == AVERROR(EAGAIN) ||
                        filter_ret == AVERROR_EOF) {
//                        LOGI("%s -> filter_ret( %d )",  __func__,filter_ret);
                        break;
                    }
                    ...
                    if (time_stamp < context->seek_to) {
                        ...
                        LOGE("(解码)audio_decode_thread => frame_pool_unref_frame");
                    } else {
                        frame_queue_put(context->audio_frame_queue, frame);
                        LOGE("(解码)audio_decode_thread => frame_queue_put");
                    }
                ...
                err[127] = 0;
                LOGE("(解码)audio_decode_thread => add to audio filter error ==>\n %s", err);
            }
//            pthread_mutex_unlock(audio_filter_context->mutex);
        } else if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
            if (ret == AVERROR(EAGAIN)) {
                LOGE("(解码)audio_decode_thread => AVERROR");
            }

            if (ret == AVERROR_EOF) {
                LOGE("(解码)audio_decode_thread => AVERROR_EOF");
            }
            AVPacket *packet = packet_queue_get(context->audio_packet_queue);
            // buffer empty ==> wait  10ms
            // eof          ==> break
            if (packet == NULL) {
                LOGE("(解码)audio_decode_thread => packet is Null");
                if (context->eof) {
                    avcodec_send_packet(context->audio_codec_context, NULL);
                    if (ret == AVERROR_EOF) {
                        LOGE("(解码)audio_decode_thread => send package is AVERROR_EOF");
//                        LOGE("context->eof");
                        usleep(BUFFER_EMPTY_SLEEP_US);
                        continue;
//                        break;
                    } else {
                        LOGE("(解码)audio_decode_thread => continue");
                        continue;
                    }
                } else {
                    usleep(BUFFER_EMPTY_SLEEP_US);
...
//                break;
            ...
            LOGE("(解码)audio_decode_thread => ret == AVERROR(EINVAL)");
            context->on_error(context, ERROR_AUDIO_DECODE_CODEC_NOT_OPENED);
            break;
        } else {
            LOGE("(解码)audio_decode_thread => else error");
...
void *video_decode_sw_thread(void *data) {
    ...
    while (!context->abort_request) {
        ...
        if (ret == 0) {
            //todo 软解
//            int seeking = isContextSeeking(context);
//            LOGI("seeking(decodeThread) seeking( %d )", seeking);
//            if (seeking >= 10 && seeking < 100) {
//                current_pts = frame->pts / 1000;
//                bool forward_seek = context->seek_to > current_pts;
//                if (forward_seek) {
//                    av_frame_unref(frame);
//                    setContextSeeking(context, 100);
//                    LOGI("seeking(decodeThread) set(100) current( %lld ) seekTo( %lld ),current video packet queue( %d ) audio packet queue( %d ) video Frame queue( %d )  audio Frame queue( %d )",
//                         current_pts,
//                         context->seek_to,
//                         context->video_packet_queue->count, context->audio_packet_queue->count,
//                         context->video_frame_queue->count, context->audio_frame_queue->count)
//                    continue;
//                }
//                setContextSeeking(context, 100);
//                LOGI("seeking(decodeThread) set(100),current video packet queue( %d ) audio packet queue( %d ) video Frame queue( %d )  audio Frame queue( %d )",
//                     context->video_packet_queue->count, context->audio_packet_queue->count,
//                     context->video_frame_queue->count, context->audio_frame_queue->count)
//            }
...
void clear_frame_queue(AVPlayContext *context) {
    ...
    while (1) {
        ...
        mediacodec_release_buffer(context, frame, false);
...
//        LOGI("%s ----->abort_request", __func__);
        ...
        //codec从output流取出数据进行操作
        ret = mediacodec_receive_frame(context, frame);

        //到底了
        if (ret == BUFFER_FLAG_END_OF_STREAM) {
            context->error_code = BUFFER_FLAG_END_OF_STREAM;
            LOGI("%s -> BUFFER_FLAG_END_OF_STREAM", __func__);
            continue;
        }

//        LOGI("%s -> get frame from mediaCodec output buffer( %d )", __func__, ret);
//        LOGI("%s -> current video writeIndex( %d )", __func__,
//             context->video_frame_queue->write_index);

        if (ret >= 0) {
            current_pts = frame->pts / 1000;
            if (context->forward_seek) {
...
//            if(afterSeek){
//                while(packet != NULL && packet != &context->video_packet_queue->flush_packet  && !(packet -> flags & AV_PKT_FLAG_KEY)){
//                    packet_pool_unref_packet(context->packet_pool, packet);
//                    packet = packet_queue_get(context->video_packet_queue);
//                    LOGI("%s searching key packet !",__func__);
//                }
//                if(packet != NULL && packet -> flags & AV_PKT_FLAG_KEY){
//                    LOGI("%s searching key packet,checkout to key packet success",__func__);
//                }else{
//                    if(packet == NULL){
//                        LOGI("%s searching key packet,checkout to key packet fail by null",__func__);
//                    }else{
//                        if(packet != &context->video_packet_queue->flush_packet){
//                            LOGI("%s searching key packet,checkout to key packet fail by no key(flush_packet)",__func__);
//                        }else{
//                            LOGI("%s searching key packet,checkout to key packet fail by no key",__func__);
//                        }
//                    }
//                }
//            }
        ...
        if (packet == NULL) {
            if (!context->end_of_stream) {
                if (context->eof) {
                    ...
                    LOGI("%s -> end_of_stream", __func__);
                } else {
                    //等待 read_thread 送入数据
                    LOGI("%s -> BUFFER_EMPTY_SLEEP_US", __func__);
                    usleep(BUFFER_EMPTY_SLEEP_US);
                    continue;
                }
            }
        ...
        // 如果 flush 过数据，则尾部数据会是 flush_packet，是一个标志，flush 之后 mediaCodec 入队列的数据一定要是关键帧
        if (packet == &context->video_packet_queue->flush_packet) {
            packet = NULL;
            LOGI("%s -> is flush_packet", __func__);
            continue;
        }

        //packet 为 read_thread 保存的数据，codec判断input流是否有可用buffer
        int buffer_index = mediacodec_dequeue_input_buffer_index(context);
//        LOGI("%s -> enable get available input buffer result( %d )", __func__, buffer_index);

        if (buffer_index >= 0) {
            //如果当前已经到可解码数据尾部，则告诉 mediaCodec 尾部已到，后续不再处理了。
            if (context->end_of_stream) {
                mediacodec_end_of_stream(context, buffer_index);
                LOGI("%s -> buffer_index ok, mediaCodec end of stream，will no resume input buffer",
                     __func__);
                continue;
            }

            if (!packet) {
//                LOGI("%s -> buffer_index ok, packet is null", __func__);
                continue;
            }
//            LOGI("%s -> buffer_index ok, current packet count(%d)", __func__,
//                 context->video_packet_queue->count);
            //packet数据写入buffer并把buffer入队列给codec解码
            ret = mediacodec_send_packet(context, packet, buffer_index);
            if (packet->flags & AV_PKT_FLAG_KEY) {
                if (afterSeek) {
                    LOGI("%s -> buffer_index ok mediaCodec dequeue input buffer result( %d ) packet pts( %lld ) I关键帧 after seek !",
                         __func__, ret, packet->pts);
                } else {
                    LOGI("%s -> buffer_index ok mediaCodec dequeue input buffer result( %d ) packet pts( %lld ) I关键帧",
                         __func__, ret, packet->pts);
                }
            } else {
                if (afterSeek) {
                    LOGI("%s -> buffer_index ok mediaCodec dequeue input buffer result( %d ) packet pts( %lld ) P/B帧 after seek !",
                         __func__, ret, packet->pts);
                } else {
//                    LOGI("%s -> buffer_index ok mediaCodec dequeue input buffer result( %d ) packet pts( %lld ) P/B帧",
//                         __func__, ret, packet->pts);
                }
                afterSeek = false;
            }
            ...
            if (0 == ret) {
                packet_pool_unref_packet(context->packet_pool, packet);
                packet = NULL;
//                LOGI("%s -> buffer_index ok, dequeue success!", __func__);
            } else {
//                LOGI("%s -> buffer_index ok, dequeue fail!", __func__);
...
//       LOGI("%s ----->", __func__);
    ...
    (*context->vm)->DetachCurrentThread(context
                                                ->vm);
...
//        LOGI("%s -->", __func__);
        ...
        if (context->status == PAUSED) {
//            LOGI("%s pause", __func__);
        }

        if (context->seeking == 1) {
            int seek_ret = av_seek_frame(context->format_context, -1, (int64_t) (context->seek_to * AV_TIME_BASE / 1000),AVSEEK_FLAG_BACKWARD);
            if (seek_ret >= 0) {
               LOGE("read seeking");
...
//            LOGI("%s buffer_size_max match， audio count %d byte % d video count %d byte %d",
//                 __func__, context->audio_packet_queue->total_bytes,
//                 context->audio_packet_queue->count, context->video_packet_queue->count,
//                 context->video_packet_queue->total_bytes)
            continue;
        }
...
//        if (packet == NULL) {
//            LOGI("%s  packet_pool_get_packet！null", __func__);
//        } else {
//            LOGI("%s  packet_pool_get_packet！no null", __func__);
//        }
        ...
        if (packet != NULL) {
            if (packet->flags & AV_PKT_FLAG_KEY) {
                if (afterSeek) {
                    LOGI("%s from file to read packet pts( %lld )  result(%d)  video(%d),audio(%d) 关键帧I after seek!",
                         __func__, packet->pts, ret, context->video_packet_queue->count,
                         context->audio_packet_queue->count);
                } else {
                    LOGI("%s from file to read packet %d 关键帧I", __func__, ret);
                }
            } else {
                if (afterSeek) {
                    LOGI("%s from file to read packet pts( %lld )  result(%d) video(%d),audio(%d) P/B帧 after seek! ",
                         __func__, packet->pts, ret, context->video_packet_queue->count,
                         context->audio_packet_queue->count);
                } else {
                    LOGI("%s from file to read packet %d P/B帧", __func__, ret);
                }
            }
            afterSeek = false;
        }
        ...
        if (ret == 0) {
            ...
            if (packet->stream_index == context->video_index) {
                context->current_video_packet_pts = av_rescale_q(packet->pts,
                                                                 ...
                                                                 AV_TIME_BASE_Q) / 1000;
                //todo
               // if(context->current_video_packet_pts>3000){
                    packet_queue_put(context->video_packet_queue, packet);
               // }
                LOGI("%s video: dts( %lld ) pts( %lld ) stream( %d ) videoPts (%lld) context:%p  readIndex : %d  writeIndex : %d count : %d duration: %lld max_duration: %lld  size: %d bufferSize ： %d status: %d",
                     __func__, packet->dts, packet->pts, packet->stream_index,
                     context->current_video_packet_pts,
                     context->format_context,
                     context->video_packet_queue->read_index,
                     context->video_packet_queue->write_index, context->video_packet_queue->count,
                     context->video_packet_queue->duration,
                     context->video_packet_queue->max_duration,
                     context->video_packet_queue->size, context->video_packet_queue->total_bytes,
                     context->status);
                packet = NULL;
            } else if (packet->stream_index == context->audio_index) {
                int64_t curPts = av_rescale_q(packet->pts,
                                              context->format_context->streams[context->audio_index]->time_base,
                                              AV_TIME_BASE_Q) / 1000;
               // if(curPts>3000){
                    packet_queue_put(context->audio_packet_queue, packet);
              //  }
                LOGI("%s audio: dts( %lld ) pts( %lld ) stream( %d ) audioPts (%lld) context:%p  readIndex : %d  writeIndex : %d count : %d  queue->duration: %lld max_duration: %lld size: %d bufferSize ： %d  status: %d",
                     __func__, packet->dts, packet->pts, packet->stream_index, curPts,
                     context->format_context,
                     context->audio_packet_queue->read_index,
                     context->audio_packet_queue->write_index, context->audio_packet_queue->count,
                     context->audio_packet_queue->duration,
                     context->audio_packet_queue->max_duration,
                     context->audio_packet_queue->size, context->audio_packet_queue->total_bytes,
                     context->status);
                packet = NULL;
            } else {
               // LOGI("%s  others index ！", __func__);
                packet_pool_unref_packet(context->packet_pool, packet);
            }
        } else if (ret == AVERROR_INVALIDDATA) {
            context->timeout_start = 0;
            packet_pool_unref_packet(context->packet_pool, packet);
           // LOGI("%s  AVERROR_INVALIDDATA！", __func__);
        } else if (ret == AVERROR_EOF) {
          //  LOGI("%s  AVERROR_EOF！", __func__);
            context->eof = true;
            if (context->status == BUFFER_EMPTY) {
                context->send_message(context, message_buffer_full);
            }
            ...
            //LOGI("%s  eof_cond_t wait ！", __func__);
           ...
           // LOGI(" %s read file error. error code ==> %d", __func__, ret);
            break;
        }
       // LOGI("%s -->", __func__);
    }
...
int av_play_play(const char *url, AVPlayContext *context) {
    ...
    LOGE("context->is_sw_decode: %d rotate: %d width: %d height: %d", context->is_sw_decode,
         context->frame_rotation, context->width, context->height);
    if (context->av_track_flags & VIDEO_FLAG) {
        if (context->is_sw_decode) {
            pthread_create(&context->video_decode_thread, &attr, video_decode_sw_thread, context);
        } else {
            pthread_create(&context->video_decode_thread, &attr,
                           video_decode_hw_thread, context);
        }
    }
    if (context->av_track_flags & AUDIO_FLAG) {
...
void av_play_seek(AVPlayContext *context, int64_t seek_to) {
    ...
    LOGE("enter: %s seek_to: %lld pre_seek: %lld duration: %lld frame_size; %d max_size; %d pd: %lld pmd: %lld vc: %d",
         __func__,
    ...
    if (context->end_of_stream) {
        mediacodec_flush(context);
        LOGE("%s end_of_stream!", __func__);
    }
    context->error_code = 0;
    context->eof = false;
    context->end_of_stream = false;
    context->forward_seek = previous_seek_time > seek_to;
    ...
    LOGI("seeking(av_play_seek)  true");
...
static int stop(AVPlayContext *context) {
    ...
    pthread_mutex_lock(&context->seek_mutex_t);
    LOGE("enter %s pthread_cond_signal", __func__);
    pthread_cond_signal(&context->seek_t);
    pthread_mutex_unlock(&context->seek_mutex_t);
...
int av_play_release(AVPlayContext *context) {
    ...
    pthread_mutex_destroy(&context->context_t);
    pthread_mutex_destroy(&context->seek_mutex_t);
    LOGE("enter %s pthread_cond_signal", __func__);
    pthread_cond_destroy(&context->eof_cond_t);
    pthread_cond_destroy(&context->seek_t);
    free(context);
    return 0;
}
