...
typedef struct AVPlayContext {
    ...
    // SEEK
    int64_t seek_to;
    int seeking;                 //默认是0，如果是后退则变成1，read_thread则需要统一处理
    bool forward_seek;            //是否是往前seek，如果是往前seek，则意味着采用 "快进跳帧模式"，如果往后seek则需要后退丢帧
    int64_t current_video_packet_pts;
    pthread_mutex_t context_t;
    pthread_cond_t seek_t;
