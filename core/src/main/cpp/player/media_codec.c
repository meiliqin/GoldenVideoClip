...
void mediacodec_release_buffer(AVPlayContext *context, AVFrame *frame,bool hasRender) {
    ...
    (*env)->CallVoidMethod(env, java_class->media_codec_object, java_class->codec_releaseOutPutBuffer,
                           (int)frame->HW_BUFFER_ID,hasRender);
...
int mediacodec_receive_frame(AVPlayContext *context, AVFrame *frame) {
    MediaCodecContext *media_codec_context = context->media_codec_context;
    if (media_codec_context == NULL) {
        LOGI("mediacodec_receive_frame context null！")
        return -1;
    }
    JNIEnv *env = NULL;
    (*context->vm)->AttachCurrentThread(context->vm, &env, NULL);
    ...
    jobject deqret = (*env)->CallObjectMethod(env, java_class->media_codec_object,
                                                       ...
                                                       (jlong) 0);
    if (!deqret) {
        LOGI("mediacodec_receive_frame codec_dequeueOutputBufferIndex -1！")
        return -1;
    }
    ...
    LOGI("mediacodec_receive_frame out_index （%d）！",out_index);
    if (out_index >= 0) {
        frame->pts = pts;
        frame->format = PIX_FMT_EGL_EXT;
        ...
        switch (out_index) {
            case -2: { //MediaCodec.INFO_OUTPUT_FORMAT_CHANGED
            ...
            case -3://MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED
                break;
            case -1://MediaCodec.INFO_TRY_AGAIN_LATER，超时
                break;
            default:
                break;
        }
...
//    int id = (*env)->CallIntMethod(env, java_class->media_codec_object, java_class->codec_dequeueInputBuffer, (jlong) 100 * 1000);
    if (buffer_index >= 0) {
        ...
        LOGE("mediacodec_send_packet input buffer id < 0  value == %zd", buffer_index);
    }
