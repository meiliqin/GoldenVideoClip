...
import com.standisland.god.media.BuildConfig
import com.standisland.god.media.encoder.MediaCodecSurfaceEncoder
import com.standisland.god.media.listener.OnExportListener
import com.standisland.god.media.util.Trinity
 ...
 */
class TrinityVideoExport(private val context: Context,private val rootPath:String) : VideoExport {
  ...
  override fun export(info: VideoExportInfo, l: OnExportListener): Int {
    ...
    //resourcePath 资源json文件根目录，确保和video_eidt中一样即可 path为输出MP4,上层随便设
    //val resourcePath = context.externalCacheDir?.absolutePath

    return export(mHandle, rootPath, info.path,
