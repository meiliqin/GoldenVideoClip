...
public class IMGImage {
    ...
    public void updateMediaPosition(long mediaPosition) {
        for (IMGSticker sticker : mBackStickers) {
            if (sticker instanceof IMGStickerView) {
                ((IMGStickerView) sticker).updateVisibility(mediaPosition);
            }
        }
    }
    ...
    private void makeMosaicBitmap() {
        ...
        if (mMode == IMGMode.MOSAIC || !isMosaicEmpty()) {
...
//                    mClipWin
    ...
    public void onDrawMosaic(Canvas canvas, int layerCount, Bitmap bitmap) {
        if (bitmap == null) {
            if (mMosaicImage != null && !mMosaicImage.isRecycled()) {
            ...
            Bitmap mosaicBitmap = buildTempMosaicBitmap(bitmap);
            canvas.drawBitmap(mosaicBitmap, null, mFrame, mMosaicPaint);
            canvas.restoreToCount(layerCount);
    ...
    private Bitmap buildTempMosaicBitmap(Bitmap bitmap) {
        int w = Math.round(bitmap.getWidth() / 64f);
        int h = Math.round(bitmap.getHeight() / 64f);
        w = Math.max(w, 8);
        h = Math.max(h, 8);
        // 马赛克画刷
        if (mMosaicPaint == null) {
...
//            mClipFrame.intersect(mFrame);
    ...
    public boolean isModify() {
        return !mHomeFrame.equals(mFrame) || !mMosaics.isEmpty() || !mDoodles.isEmpty() || !mBackStickers.isEmpty() || mRotate != 0 || mForeSticker != null;
    }

    public boolean isClip() {
        return !mHomeFrame.equals(mFrame);
    }

    public List<IMGSticker> getBackStickers() {
        return mBackStickers;
    }
}
